...
class Pascal3DPlus(Dataset):
    def __init__(self, transforms, enable_cache=True, **kwargs):
        ...
        self.corruption = kwargs['corruption']
    ...
    def __getitem__(self, item):
        ...
        if self.corruption != 'normal':
            img = Image.fromarray(corrupt(np.array(img), corruption_name=self.corruption, severity=4))
        # img.save('results/snow/{}.png'.format(name_img))
        # img = Image.fromarray(corrupt(np.array(img), \
