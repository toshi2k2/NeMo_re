...
parser.add_argument('--turn_off_clutter', default=True, type=bool, help='')
...
parser.add_argument('--corruptions', default='', type=str, help='type of corruption - normal, snow, etc.')
parser.add_argument('--rotpred_path', default='./junk/', type=str, \
    help='Rotation matrix predictions/image (.npy) path')
...
os.makedirs(args.rotpred_path+'/%s/' % args.corruptions, exist_ok=True) # needs to be fixed
...
if __name__ == '__main__':
    ...
    for k, subtype in enumerate(subtypes):
        ...
        error_im, total_im = 0, len(name_list.tolist())
        ...
        for image_name in name_list:
            ...
            for epoch in range(epochs):
                try:
                    ...
                    print("Epoch:", epoch, args.type_, image_name)
                    err_signal = True
                    error_im+=1
                    if error_im>.1*total_im:
                        # print("%s too many errors. EXITING!"%args.type_)
                        exit("%s too many errors. EXITING!"%args.type_)
                    optim.zero_grad()
                    break
                object_score = torch.sum(projected_map * predicted_map, dim=0)
            ...
            # np.save('/home/adam/NeMo/junk/snow/' + image_name + '.npy', (theta_pred, distance_pred, elevation_pred, azimuth_pred))
            np.save(args.rotpred_path+'/'+args.corruptions+'/' + image_name + '.npy', records)  # add argument for this
            ...
            if np.any(np.isnan(anno_matrix)) or np.any(np.isnan(pred_matrix)) or np.any(np.isinf(anno_matrix)) or np.any(np.isinf(pred_matrix)):
                ...
                try:
                    ...
                    print("%s Skipping: %s" % (args.type_ ,image_name))
                    print(e)
                    os.remove(args.rotpred_path+'/'+args.corruptions+'/' + image_name + '.npy')
                    continue
