...
from dataset.Pascal3DPlus import ToTensor, Normalize, Pascal3DPlus, UnNormalize
...
parser.add_argument('--eval_kp_score', default=True, type=bool)
...
parser.add_argument('--corruptions', default='normal', help='normal, snow, etc.')
parser.add_argument('--mod_im_path', default='./results/', type=str, \
    help='Corrupted input image visualization path')

args = parser.parse_args()
assert(args.corruptions!='')
...
for i, (n, subtype) in enumerate(zip(n_list, subtypes)):
    ...
    os.makedirs(args.mod_im_path+'/%s/' % (args.corruptions), exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for j, sample in enumerate(Pascal3D_dataloader):
            ...
            # saving images
            if args.corruptions == 'normal':
                if j%1000==0:
                    im = torchvision.transforms.functional.to_pil_image(u(img[0]))
                    # print(type(im))
                    # im.show()
                    im.save(args.mod_im_path+'/%s/%s.png' % (args.corruptions, this_name[0]))
                    del im
            else:
                for imx, nmx in zip(img, this_name):
                    im = torchvision.transforms.functional.to_pil_image(u(imx))
                    im.save(args.mod_im_path+'/%s/%s.png' % (args.corruptions, nmx)) # make this into argument
                    del im
