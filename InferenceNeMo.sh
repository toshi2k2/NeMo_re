...
DATAROOT="${ROOT}/data"
EXPROOT="${ROOT}/exp_gaussb_clutter"
...
# ALL_CATEGORIES=("bicycle" "bus" "car" "motorbike" "train") # for classes which run into error
...
elif [ "${#GPU_LIST[@]}" -eq "2" ]; then
    # 2 GPU SETTING
    # GPU_ASSIGNMENT=("0"       "0"        "1"     "0"       "1"    "3"    "1"      "2"            "2"          "2"     "1"      "2")
    GPU_ASSIGNMENT=("${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[1]}" "${GPU_LIST[0]}" "${GPU_LIST[1]}" "${GPU_LIST[1]}" "${GPU_LIST[1]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[1]}" "${GPU_LIST[1]}" "${GPU_LIST[0]}")
else
    # 1 GPU SETTING
    # GPU_ASSIGNMENT=("0"       "0"        "1"     "0"       "1"    "3"    "1"      "2"            "2"          "2"     "1"      "2")
    GPU_ASSIGNMENT=("${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}" "${GPU_LIST[0]}")
fi
