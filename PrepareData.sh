...
python ./code/dataset/CreatePascal3DNeMo.py --overwrite True --source_path "${PATH_PASCAL3DP}" \
...
if ${ENABLE_OCCLUDED}; then
    for OCC_LEVEL in "${OCC_LEVELS[@]}"; do
        ...
        python ./code/dataset/CreatePascal3DNeMo.py --overwrite True \
...
for MESH_D in "${MESH_DIMENSIONS[@]}"; do
    python ./code/dataset/generate_3Dpascal3D.py --overwrite True \
